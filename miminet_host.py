        ...
        if job_id:
            ...
            if job_level < 20:
                ...
                if job_id == 3:
                    ...
                    job_3_arg_3 = request.form.get('config_host_send_udp_data_port_input_field')
                    ...
                    if not job_3_arg_2:
                        ret.update({'warning': 'Не указан IP адрес для команды "Отправить данные (UDP)"'})
                        return make_response(jsonify(ret), 200)

                    if not job_3_arg_3:
                        ret.update({'warning': 'Не указан порт для команды "Отправить данные (UDP)"'})
                        return make_response(jsonify(ret), 200)

                    if int(job_3_arg_3) < 0 or int(job_3_arg_3) > 65535:
                        ret.update({'warning': 'Неверно указан порт для команды "Отправить данные (UDP)"'})
                        return make_response(jsonify(ret), 200)
    ...
    if request.method == "POST":
        ...
        if job_id:
            ...
            if job_level < 20:
                ...
                # Start UDP server
                if job_id == 200:
                    job_200_arg_1 = request.form.get('config_server_start_udp_server_ip_input_field')
                    job_200_arg_2 = request.form.get('config_server_start_udp_server_port_input_field')

                    if not job_200_arg_1:
                        ret.update({'warning': 'Не указан IP адрес для команды "Запустисть UDP сервер"'})
                        return make_response(jsonify(ret), 200)

                    if not job_200_arg_2:
                        ret.update({'warning': 'Не указан порт для команды "Запустисть UDP сервер"'})
                        return make_response(jsonify(ret), 200)

                    if int(job_200_arg_2) < 0 or int(job_200_arg_2) > 65535:
                        ret.update({'warning': 'Неверно указан порт для команды "Запустить UDP сервер"'})
                        return make_response(jsonify(ret), 200)
                    ...
                    try:
                        ...
                        jnet['jobs'].append({'id': job_id_generator(),
                                            'level': job_level,
                                             ...
                                             'print_cmd': 'nc -u ' + str(job_200_arg_1) + ' -l ' + str(job_200_arg_2)})
                    except Exception as e:
                        print(e)
